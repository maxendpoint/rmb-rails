...
multi-threaded EJB operating system, the container, to subscribe to brokers and, when a message arrive, create a Message Bean which 
then started a Session Bean (think of a rails controller) to instantiate one or more Entity Beans (think of a rails model) 
...
to get incoming message broker subscriptions injected directly into the main Rails data stream.  So, how can we convert an MQ message 
...
that broker, and then forward them on to a standard Rails controller using the standard REST protocol that is well supported by Rails.  
